...
def negSamplingLossAndGradient(
    ...
    pos_sigmoid = sigmoid(score)
    loss += -np.log(pos_sigmoid)

    # centerWordVec Grad
    gradCenterVec += (pos_sigmoid-1) * u_o
    ...
    for negIdx in negSampleWordIndices:
        u_k = outsideVectors[negIdx]  # Negative word vector
        neg_score = np.dot(u_k, centerWordVec)  # Dot product with center word
        neg_sigmoid = sigmoid(-neg_score)  # Sigmoid of the negative score
        loss += -np.log(neg_sigmoid)  # Add negative sample loss
        
        # Gradient for center word vector
        gradCenterVec += (1 - neg_sigmoid) * u_k
        
        # Gradient for negative word vector
        gradOutsideVecs[negIdx] += (1 - neg_sigmoid) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # Extract the center word vector from the centerWordVectors
    centerWordVec = centerWordVectors[centerWordIdx]
    
    # Loop over each outside word in the context window (outsideWords)
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]  # Get the index of the outside word
        
        # Compute the loss and gradients using the provided loss and gradient function
        current_loss, gradCenter, gradOutside = word2vecLossAndGradient(
        ...
        # Accumulate the gradient for the center word
        gradCenterVecs[centerWordIdx] += gradCenter
        
        # Accumulate the gradients for the outside words
